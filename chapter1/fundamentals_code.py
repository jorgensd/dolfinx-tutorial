...
# Note that in addition to specifying how many elements we would like to have in each direction,
...
# $ u = \sum*{j=1}^N U_j\phi_j.$
# By writing `problem.solve()` we compute all the coefficients $U_1,\dots, U_N$. These values are known as the \_degrees of freedom* (dofs). We can access the degrees of freedom by accessing the underlying vector in `uh`.
